    ...
    def buildFiringSolution(self):
        ...
        for target in self.targets[::-1]:
            ...
            if not offers:
                ...
                # How much more influence does delegate and passworder need?
                # How much more influence would we need to chew through? 
                # Both good questions.
                return False

            else:
                hitman = min(offers, key=lambda x: x[1])
                print(f"RO {hitman[0]} will take {target.name}, for a bid of {hitman[1]}")
...
#        print(f"Transitioning with the resultant state would require {transCost} influence")
        ...
        firingSolution.buildFiringSolution()
